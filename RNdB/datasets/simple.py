# Added for testing the simple dataset
import os
import numpy as np
import pandas as pd
import json
...
class Simple(Dataset):
    ...
    def get_dataset(self):
        ...
        self.df = pd.read_csv(os.path.join(data_location, "simple.csv"))
        self.domain = json.load(open(os.path.join(data_location, "simple-domain.json")))
...
if __name__ == "__main__":
    simple = Simple(False, "../data/", False, 1, 5)

    X = simple.get_dataset()

    # generate random k-way attributes tuples at random
    kway_marginals = simple.randomKway(num_kways=3, k=1)
    print(X.shape, len(kway_marginals), kway_marginals[0])

    # generate random N 3-way queries of indexes from kway_marginals
    kway_queries, num_queries = simple.get_queries(kway_marginals)
