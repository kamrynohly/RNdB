...
class CSCourse(Dataset):
    ...
    def get_dataset(self):
        ...
        self.df = pd.read_csv(os.path.join(data_location, "CScourse.csv"))
        self.domain = json.load(open(os.path.join(data_location, "CScourse-domain.json")))
...
if __name__ == "__main__":
    csCourse = CSCourse(False, "data/", False, 1, 5)

    X = csCourse.get_dataset()

    # generate random k-way attributes tuples at random
    kway_marginals = csCourse.randomKway(num_kways=128, k=3)
    print(X.shape, len(kway_marginals), kway_marginals[0])

    # generate random N 3-way queries of indexes from kway_marginals
    kway_queries, num_queries = csCourse.get_queries(kway_marginals)
    print(
        sum([reduce(lambda x, y: x * y, [len(i) for i in q], 1) for q in kway_queries]),
        num_queries,
    )

    kway_queries, num_queries = csCourse.get_queries(kway_marginals, N=-1)
